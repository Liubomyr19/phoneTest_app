...
export const Card: React.FC<Props> = ({ item, discount }) => {
  ...
  const handleBtnClick = (card: Product, action: string) => {
    ...
    const index = currentProducts.findIndex(product => product.id === card.id);

    if (!updatedCard.inCart && !updatedCard.inFavourite) {
      currentStoreState = currentStoreState.filter(
        product => product.id !== updatedCard.id,
      );
    } else {
      const indexStore = currentStoreState.findIndex(
        storeItem => storeItem.id === card.id,
      );
  ...
  return (
    <div className="product-card">
      ...
      <div className="product-card_price">
        ...
        {discount && (
          <p className="product-card_price_old">
            <s>{`$${item.fullPrice}`}</s>
          </p>
        )}
      </div>
