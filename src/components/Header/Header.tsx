...
const getLinkClassLeft = ({ isActive }: { isActive: boolean }) =>
  cn({
...
const getLinkClassIcons = ({ isActive }: { isActive: boolean }) =>
  cn({
...
export const Header = () => {
  ...
  if (itemsInCart.length > 0) {
    totalInCart = itemsInCart.map(prod => prod.count).reduce((a, b) => a + b);
  }
    ...
    <>
      <header className="header">
        ...
        <div className="header_right-box">
          ...
          <NavLink to="/favourites" className={getLinkClassIcons}>
            <img
          ...
          <NavLink to="/cart" className={getLinkClassIcons}>
            <img src={ICONS.iconCart} alt="Icon cart" className="icon" />
            {totalInCart > 0 && (
